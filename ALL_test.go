...
func TestALL(t *testing.T) {
	...
	flag.Parse()
	opt.Paths = flag.Args()
	result := godog.RunWithOptions("TestALL", func(s *godog.Suite) { FeatureContext(s) }, opt)
	if result != 0 {
...
func FeatureContext(s *godog.Suite) {
	...
	s.Step(`^I import the file "([^"]*)"$`, iImportTheFile)
	s.Step(`^I keep the selected node\'s data$`, iKeepTheSelectedNodesData)
	s.Step(`^the kept data should be "([^"]*)"$`, theKeptDataShouldBe)
